...
for i=1:(m1-1)
    for j=1:(n1-1)
          term1(i,j)=(Im3(i,j+dx)*duxI(i,j+dx)-Im3(i,j)*duxI(i,j))/dx;  %根据速度场小数部分duxI，通过差分近似计算水平方向的梯度，并结合图像像素之间的差值，得到term1
          term2(i,j)=(Im3(i+dy,j)*duyI(i+dy,j)-Im3(i,j)*duyI(i,j))/dy;   
          Im1_shift1(i,j)=Im3(i,j)-(term1(i,j)+term2(i,j))*dt;  %通过当前像素点(i, j)处的像素值，减去根据速度场计算得到的梯度信息乘以时间步长dt后的值，结果赋给Im1_shift1(i,j)
    end
end
